...
class LoadProfile:
    def __init__(self, args, time_interval, steps_ori, steps_use, dss_folder_path, dss_file, worker_idx=None):
        self.steps_ori = steps_ori
        ...
        self.LOAD_NAMES = sorted(self.LOAD_NAMES)
        print('[loadprofile] load names:', self.LOAD_NAMES)
        ...
        if args.gen_ls_csv and len(self.FILES) == 0:
            ori_files = os.listdir(self.loadshape_path)
            for f_ in ori_files:
                for i in self.LOAD_NAMES:
                    if 'pv' in i and f_ == 'basepvls.csv':
                        src_file = os.path.join(self.loadshape_path, f_)
                        dest_file = os.path.join(self.loadshape_path, 'loadshape_' + i + '.csv')
                        shutil.copy2(src_file, dest_file)
                    if 'pv' not in i and f_ == 'basels.csv':
                        src_file = os.path.join(self.loadshape_path, f_)
                        dest_file = os.path.join(self.loadshape_path, 'loadshape_' + i + '.csv')
                        shutil.copy2(src_file, dest_file)
    ...
    def gen_loadprofile(self, scale=1.0):
        ...
        episodes = len(df) // (self.steps_ori * len(self.LOAD_NAMES))

        # stop here only if the loadprofile folders exist
        checks0 = [fnmatch(f, '0*') for f in os.listdir(self.loadshape_path)]
        checks1 = [fnmatch(f, '1*') for f in os.listdir(self.loadshape_path)]
        checks2 = [fnmatch(f, '2*') for f in os.listdir(self.loadshape_path)]
        checks3 = [fnmatch(f, '3*') for f in os.listdir(self.loadshape_path)]
        checks = checks0 + checks1 + checks2 + checks3
        ...
        if sum(checks) != 0 and fscale == scale:
            episodes = sum(checks)
            return episodes
        ...
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        print('check # of all days for generate:', episodes)
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        ...
        for i in range(self.steps_ori * episodes * len(self.LOAD_NAMES)):
            load_col.append(self.LOAD_NAMES[i // (self.steps_ori * episodes)])
            episode_col.append((i // self.steps_ori) % episodes)
            step_col.append(i % self.steps_ori)
        load_col = sorted(load_col)
        # print(load_col)
        # time.sleep(10)
        df = df[:len(load_col)]
        df['load'] = load_col
        ...
        # ------------------------------------------------------------------------
        df_index = int(self.steps_ori / self.steps_use)
        df_use = df.iloc[0:len(df):df_index]

        step_col_new, timeinterval_col = [], []
        for i in range(len(df_use)):
            step_col_new.append(i % self.steps_ori)
            timeinterval_col.append(self.time_interval * df_index)
        df_use['step'] = step_col_new
        df_use['time_interval_weuse'] = timeinterval_col
        print('[loadprofile-gen_loadprofile]')
        print(df_use)
        # ------------------------------------------------------------------------
        ...
        for episode in range(episodes):
            if not os.path.exists(os.path.join(self.loadshape_path, str(episode).zfill(3))):
                ...
                sdf = df_use[df_use['episode'] == episode]
    ...
    def choose_loadprofile(self, idx):  # yb: use idx(load_profile_idx) to write loadshape into dss from csv
        ...
        with open(os.path.join(self.dss_folder_path, self.loadshape_dss), 'w') as fp:
            for load_name in self.LOAD_NAMES:
                fp.write(
                    f'New Loadshape.loadshape_{load_name} npts={self.steps_use} sinterval={60 * 60 * 24 // self.steps_use} ' +
