...
class LSTMPolicy(Policy):
    ...
    def __init__(self, policy_params):
        ...
        if "weights" in policy_params:
            ...
            self.weights = 0.01 * (np.random.rand(num_weights)-0.5)
            # self.weights = 0.0000001 * (np.random.rand(num_weights)-0.5)
    ...
    """
        ...
        W_f = np.reshape(self.weights[self._lstm_weight_start_idx[0]:self._lstm_weight_end_idx[0]],
                         (self._lstm_weight_size[0], self._lstm_weight_size[1]))
        W_i = np.reshape(self.weights[self._lstm_weight_start_idx[1]:self._lstm_weight_end_idx[1]],
                         (self._lstm_weight_size[0], self._lstm_weight_size[1]))
        W_c = np.reshape(self.weights[self._lstm_weight_start_idx[2]:self._lstm_weight_end_idx[2]],
                         (self._lstm_weight_size[0], self._lstm_weight_size[1]))
        W_o = np.reshape(self.weights[self._lstm_weight_start_idx[3]:self._lstm_weight_end_idx[3]],
                         (self._lstm_weight_size[0], self._lstm_weight_size[1]))

        ft__ = np.concatenate([self.hidden_h, ob])
        t = 0
        # print(ft__.shape)  # (174,)
        # print(W_f.shape)  # (174,64)
        ft_ = np.dot(ft__, W_f)  # this line consume time mostly, but may doesn't matter with code itself
        ...
        return normalized_actions, t  # actions
