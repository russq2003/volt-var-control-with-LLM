...
# check dss API: https://dss-extensions.org/python_apis.html

import dss as opendss  # yb: use DSS-Python rather than OpenDSSDirect.py
...
class Circuits():
    def __init__(self, dss_file, args,
        ...
        self.bus_obj = dict()  # map from bus name to the objects(load,capacitor,batteries,PVSystems) on the bus
    ...
    def reset(self):
        ...
        self.set_all_capacitor_statuses([0] * len(self.capacitors), change_dss=False)

        self.set_all_PVSystems_Q([0] * len(self.pvsystems), 0, 0, change_dss=False)

        # print(self.dss.ActiveCircuit.Loads.Name, self.dss.ActiveCircuit.Loads.Next, self.dss.ActiveCircuit.Loads.kW)

        for bat in self.batteries.keys():
            self.batteries[bat].reset()
        # self.dss.ActiveCircuit.Solution.Solve()
        # self.dss.ActiveCircuit.Solution.StepSize = time_inter * 60
        # self.dss.ActiveCircuit.Solution.SolveNoControl()  # avoid to use loadshape
    ...
    def initialize(self, noWei=True):
        ...
        # self.dss.ActiveCircuit.Solution.Solve()
        # print(self.dss.ActiveCircuit.PVSystems.AllNames)
        self.dss.ActiveCircuit.Solution.SolveNoControl()
    ...
    def set_all_batteries_before_solve(self, nkws_or_states, change_dss=True):
        ...
        if self.bat_act_num == np.inf:
            nkws_or_states = np.array(nkws_or_states, dtype=np.float32)  # continuous
        else:
            nkws_or_states = np.array(nkws_or_states, dtype=int)  # discrete
...
#             print('kw:',kw)
    ...
    def set_all_PVSystems_Q(self, PVpercent, max_PV_Q, t, change_dss=True):
        # print(change_dss)
        # assert len(PVpercent) > 0 and len(PVpercent) == len(self.pvsystems), 'inconsistent states'
        # print(PVpercent)
        PVpercent = np.array(PVpercent, dtype=np.float32)
        PV_Q = PVpercent * max_PV_Q

        PV2kvar = dict()
        for i, PV in enumerate(self.pvsystems.keys()):
            PVS = self.pvsystems[PV]
            PVS.kvar = PV_Q[i]
            # print(PVS.name[:])
            PV2kvar[PVS.name[:]] = PVS.kvar
            # print(PVS.kvar)

        ## change kw in dss
        if change_dss:
            dssPV = self.dss.ActiveCircuit.Loads
#             print(self.dss.ActiveCircuit.CktElements('Generator.batt1').TotalPowers)
#             print('yb')
#             print(dssPV.Name)
#             print(dssPV.kW)
#             print(self.dss.ActiveCircuit.CktElements('Load.'+dssPV.Name).TotalPowers)
            if dssPV.First == 0: return  # no such kind of object
            while True:
                if dssPV.Name in PV2kvar:
#                     print(dssPV.Name)
#                     print(dssPV.kW)
#                     print(self.dss.ActiveCircuit.CktElements('Load.'+dssPV.Name).TotalPowers)
                    kvar = PV2kvar[dssPV.Name]
                    dssPV.kvar = -kvar
                    if self.args.PVmode == 2:
                        dssPV.kW = 0
                        # print(dssPV.kW, dssPV.kvar)
                        # dssPV.kV = 0
                    # print(kvar, dssPV.kvar)
    ...
    def set_all_batteries_after_solve(self):
        ...
        for i, bat in enumerate(self.batteries):
            batt = self.batteries[bat]
#             print('act:',batt.actual_power())
#             batt.kwh = np.float64(batt.kwh)
            batt.kwh += batt.actual_power() * batt.duration
            # enforce capacity constraint and round to integer
            batt.kwh = round(max(0.0, min(batt.max_kwh, batt.kwh)), 3)
            batt.soc = (batt.kwh+(batt.max_kwh/0.8)*0.1) / (batt.max_kwh/0.8)
    ...
    def _gen_PV_obj(self):
        dssPV = self.dss.ActiveCircuit.Loads
        if dssPV.First == 0: return  # no PV in this env
        while True:
            if 'pv' in dssPV.Name:  # Robert: find PV from loads
                PVname = dssPV.Name
                # print(PVname)
                BusNames = self.dss.ActiveCircuit.CktElements.BusNames[0].split('.')
                bus = BusNames[0]
                if len(BusNames) > 1:
                    ...
                    phases = self.bus_phase[bus]
                fea = [dssPV.kvar]
                self.add_PVSystems(PVname, bus, phases, fea)
            if dssPV.Next == 0: break
    ...
    def add_PVSystems(self, PVname, bus, phases, feature):
        self.pvsystems[PVname] = PVSystem(PVname, bus, phases, feature)
        if bus not in self.bus_obj:
...
############# Node Objects #############(with only 1 terminal)
class Node():
...
class Battery(Node):
    def __init__(self, dss, batname, bus1, phases, feature, bat_act_num=33):
        ...
        self.dss.ActiveCircuit.Solution.StepSize = 300
        ...
        self.duration = self.dss.ActiveCircuit.Solution.StepSize / (3600.0)  # time step in hour(5min)
#         print(self.duration)
#         self.duration = self.dss.ActiveCircuit.Solution.StepSize 
